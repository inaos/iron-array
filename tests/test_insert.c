...
static ina_rc_t test_insert(iarray_context_t *ctx, iarray_container_t *c_x, void *buffer, int64_t buffersize,
                            int64_t axis, int64_t start) {

    INA_TEST_ASSERT_SUCCEED(iarray_container_insert(ctx, c_x, buffer, buffersize, axis, start));

    return INA_SUCCESS;
}

static ina_rc_t _execute_iarray_insert(iarray_context_t *ctx, iarray_data_type_t dtype, int64_t type_size, int8_t ndim,
                                      const int64_t *shape, const int64_t *cshape, const int64_t *bshape,
                                      int64_t axis, int64_t start, int64_t buffer_len, bool contiguous, char *urlpath) {
    ...
    INA_TEST_ASSERT_SUCCEED(test_insert(ctx, c_x, buffer, buffer_len * type_size, axis, start));

    int64_t start_[IARRAY_DIMENSION_MAX] = {0};
    start_[axis] = start;
    int64_t stop[IARRAY_DIMENSION_MAX];
    int64_t buffershape[IARRAY_DIMENSION_MAX];
    int64_t buffersize = type_size;
    for (int i = 0; i < ndim; ++i) {
        if (i != axis) {
            stop[i] = shape[i];
            buffershape[i] = shape[i];
            buffersize *= buffershape[i];
        }
    }
    buffershape[axis] = buffer_len / (buffersize / type_size);
    buffersize *= buffershape[axis];
    stop[axis] = start + buffershape[axis];

    /* Fill buffer with a slice from the new chunks */
    uint8_t *res_buffer = ina_mem_alloc(buffersize);
    INA_TEST_ASSERT_SUCCEED(iarray_get_slice_buffer(ctx, c_x, start_, stop, res_buffer, buffersize));
...
INA_TEST_DATA(insert) {
    iarray_context_t *ctx;
};

INA_TEST_SETUP(insert) {
    iarray_init();
...
INA_TEST_TEARDOWN(insert) {
    iarray_context_free(&data->ctx);
    iarray_destroy();
}

INA_TEST_FIXTURE(insert, 2_d) {
    ...
    INA_TEST_ASSERT_SUCCEED(_execute_iarray_insert(data->ctx, dtype, type_size, ndim, shape, cshape, bshape, axis,
                                                   start, buffer_len, false, NULL));
}

INA_TEST_FIXTURE(insert, 3_f) {
    ...
    INA_TEST_ASSERT_SUCCEED(_execute_iarray_insert(data->ctx, dtype, type_size, ndim, shape, cshape, bshape, axis,
                                                   start, buffer_len, false, "arr.iarr"));
}

INA_TEST_FIXTURE(insert, 5_i) {
    ...
    INA_TEST_ASSERT_SUCCEED(_execute_iarray_insert(data->ctx, dtype, type_size, ndim, shape, cshape, bshape, axis,
                                                   start, buffer_len, true, NULL));
}

INA_TEST_FIXTURE(insert, 5_s) {
    ...
    INA_TEST_ASSERT_SUCCEED(_execute_iarray_insert(data->ctx, dtype, type_size, ndim, shape, cshape, bshape, axis,
                                                   start, buffer_len, true, "arr.iarr"));
}

INA_TEST_FIXTURE(insert, 4_ull) {
