...
ina_rc_t _iarray_reduce2(iarray_context_t *ctx,
    ...
    switch (func) {
        case IARRAY_REDUCE_VAR:
            ...
            switch (a->dtshape->dtype) {
                case IARRAY_DATA_TYPE_DOUBLE:
                    reduce_function = &REDUCTION(VAR, double);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_FLOAT:
                    reduce_function = &REDUCTION(VAR, float);
                    dtype = IARRAY_DATA_TYPE_FLOAT;
                    break;
                case IARRAY_DATA_TYPE_INT64:
                    reduce_function = &REDUCTION(VAR, int64_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_INT32:
                    reduce_function = &REDUCTION(VAR, int32_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_INT16:
                    reduce_function = &REDUCTION(VAR, int16_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_INT8:
                    reduce_function = &REDUCTION(VAR, int8_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_UINT64:
                    reduce_function = &REDUCTION(VAR, uint64_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_UINT32:
                    reduce_function = &REDUCTION(VAR, uint32_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_UINT16:
                    reduce_function = &REDUCTION(VAR, uint16_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_UINT8:
                    reduce_function = &REDUCTION(VAR, uint8_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_BOOL:
                    reduce_function = &REDUCTION(VAR, bool);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                default:
        ...
        case IARRAY_REDUCE_NAN_VAR:
            ...
            switch (a->dtshape->dtype) {
                case IARRAY_DATA_TYPE_DOUBLE:
                    reduce_function = &NANREDUCTION(VAR, double);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_FLOAT:
                    reduce_function = &NANREDUCTION(VAR, float);
                    dtype = IARRAY_DATA_TYPE_FLOAT;
                    break;
                default:
                    IARRAY_TRACE1(iarray.error, "Invalid dtype");
        ...
        case IARRAY_REDUCE_NAN_STD:
            ...
            switch (a->dtshape->dtype) {
                case IARRAY_DATA_TYPE_DOUBLE:
                    reduce_function = &NANREDUCTION(STD, double);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_FLOAT:
                    reduce_function = &NANREDUCTION(STD, float);
                    dtype = IARRAY_DATA_TYPE_FLOAT;
                    break;
                default:
                    IARRAY_TRACE1(iarray.error, "Invalid dtype");
        ...
        case IARRAY_REDUCE_STD:
            ...
            switch (a->dtshape->dtype) {
                case IARRAY_DATA_TYPE_DOUBLE:
                    reduce_function = &REDUCTION(STD, double);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_FLOAT:
                    reduce_function = &REDUCTION(STD, float);
                    dtype = IARRAY_DATA_TYPE_FLOAT;
                    break;
                case IARRAY_DATA_TYPE_INT64:
                    reduce_function = &REDUCTION(STD, int64_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_INT32:
                    reduce_function = &REDUCTION(STD, int32_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_INT16:
                    reduce_function = &REDUCTION(STD, int16_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_INT8:
                    reduce_function = &REDUCTION(STD, int8_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_UINT64:
                    reduce_function = &REDUCTION(STD, uint64_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_UINT32:
                    reduce_function = &REDUCTION(STD, uint32_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_UINT16:
                    reduce_function = &REDUCTION(STD, uint16_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_UINT8:
                    reduce_function = &REDUCTION(STD, uint8_t);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_BOOL:
                    reduce_function = &REDUCTION(STD, bool);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                default:
                    IARRAY_TRACE1(iarray.error, "Invalid dtype");
        ...
        case IARRAY_REDUCE_NAN_MEAN:
            switch (a->dtshape->dtype) {
                case IARRAY_DATA_TYPE_DOUBLE:
                    reduce_function = &NANREDUCTION(MEAN, double);
                    dtype = IARRAY_DATA_TYPE_DOUBLE;
                    break;
                case IARRAY_DATA_TYPE_FLOAT:
                    reduce_function = &NANREDUCTION(MEAN, float);
                    dtype = IARRAY_DATA_TYPE_FLOAT;
                    break;
                default:
                    IARRAY_TRACE1(iarray.error, "Invalid dtype");
        ...
        case IARRAY_REDUCE_ALL:
            switch (a->dtshape->dtype) {
                case IARRAY_DATA_TYPE_DOUBLE:
                    reduce_function = &ONESHOTREDUCTION(ALL, double);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_FLOAT:
                    reduce_function = &ONESHOTREDUCTION(ALL, float);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_INT64:
                    reduce_function = &ONESHOTREDUCTION(ALL, int64_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_INT32:
                    reduce_function = &ONESHOTREDUCTION(ALL, int32_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_INT16:
                    reduce_function = &ONESHOTREDUCTION(ALL, int16_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_INT8:
                    reduce_function = &ONESHOTREDUCTION(ALL, int8_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_UINT64:
                    reduce_function = &ONESHOTREDUCTION(ALL, uint64_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_UINT32:
                    reduce_function = &ONESHOTREDUCTION(ALL, uint32_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_UINT16:
                    reduce_function = &ONESHOTREDUCTION(ALL, uint16_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_UINT8:
                    reduce_function = &ONESHOTREDUCTION(ALL, uint8_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_BOOL:
                    reduce_function = &ONESHOTREDUCTION(ALL, bool);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                default:
                    IARRAY_TRACE1(iarray.error, "Invalid dtype");
        ...
        case IARRAY_REDUCE_ANY:
            switch (a->dtshape->dtype) {
                case IARRAY_DATA_TYPE_DOUBLE:
                    reduce_function = &ONESHOTREDUCTION(ANY, double);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_FLOAT:
                    reduce_function = &ONESHOTREDUCTION(ANY, float);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_INT64:
                    reduce_function = &ONESHOTREDUCTION(ANY, int64_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_INT32:
                    reduce_function = &ONESHOTREDUCTION(ANY, int32_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_INT16:
                    reduce_function = &ONESHOTREDUCTION(ANY, int16_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_INT8:
                    reduce_function = &ONESHOTREDUCTION(ANY, int8_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_UINT64:
                    reduce_function = &ONESHOTREDUCTION(ANY, uint64_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_UINT32:
                    reduce_function = &ONESHOTREDUCTION(ANY, uint32_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_UINT16:
                    reduce_function = &ONESHOTREDUCTION(ANY, uint16_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_UINT8:
                    reduce_function = &ONESHOTREDUCTION(ANY, uint8_t);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                case IARRAY_DATA_TYPE_BOOL:
                    reduce_function = &ONESHOTREDUCTION(ANY, bool);
                    dtype = IARRAY_DATA_TYPE_BOOL;
                    break;
                default:
                    IARRAY_TRACE1(iarray.error, "Invalid dtype");
