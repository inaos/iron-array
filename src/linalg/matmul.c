...
static void _c_matmul(double *a, double *b, double *c, int64_t M, int64_t N, int64_t K) {
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < N; ++j) {
            c[i * N + j] = 0;
            for (int k = 0; k < K; ++k) {
...
static int _iarray_matmul_prefilter(blosc2_prefilter_params *pparams) {
    ...
    // Compute block info
    int32_t start[2] = {0};
    _iarray_prefilter_block_info(matmul_params->c, pparams->out_offset, pparams->out_size, start);
...
static ina_rc_t iarray_linalg_matmul_blosc(iarray_context_t *ctx,
    ...
    while (nchunk < c->catarr->extnitems / c->catarr->chunknitems) {
        ...
        if (chunk_row != chunk_index[1]) {
            ...
            _iarray_repart_caches(ctx, shape_b[0], shape_b[1], c->catarr->blockshape[1], c->catarr->itemsize,
                                  cache_aux_b, cache_b);
