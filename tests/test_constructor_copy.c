                           ...
                           double stop)
{
    ...
    INA_TEST_ASSERT_SUCCEED(iarray_copy(ctx, c_x, NULL, 0, false, &c_y));

    // Assert iterator reading it
    iarray_iter_read_t *I2;
    iarray_iter_read_value_t val;
    INA_TEST_ASSERT_SUCCEED(iarray_iter_read_new(ctx, &I2, c_y, &val));
...
INA_TEST_FIXTURE(constructor_copy, 2_d_p) {
    ...
    INA_TEST_ASSERT_SUCCEED(test_copy(data->ctx, dtype, ndim, shape, pshape, start, stop));
}

INA_TEST_FIXTURE(constructor_copy, 2_f) {
...
INA_TEST_FIXTURE(constructor_copy, 5_d) {
    iarray_data_type_t dtype = IARRAY_DATA_TYPE_DOUBLE;
...
INA_TEST_FIXTURE(constructor_copy, 7_f_) {
    iarray_data_type_t dtype = IARRAY_DATA_TYPE_FLOAT;
