...
int main(void)
{
    ...
    IARRAY_RETURN_IF_FAILED(iarray_to_buffer(ctx, c_x, b_x, size_x * sizeof(double)));
    IARRAY_RETURN_IF_FAILED(iarray_to_buffer(ctx, c_y, b_y, size_y * sizeof(double)));
    ...
    INA_STOPWATCH_START(w);
    IARRAY_RETURN_IF_FAILED(iarray_linalg_matmul(ctx, c_x, c_y, &store_z, &c_z));
    INA_STOPWATCH_STOP(w);
    IARRAY_RETURN_IF_FAILED(ina_stopwatch_duration(w, &elapsed_sec));

    printf("Time iarray: %.4f\n", elapsed_sec);

    IARRAY_RETURN_IF_FAILED(iarray_to_buffer(ctx, c_z, b_res, size_z * sizeof(double)));
