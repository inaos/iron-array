...
int main(int argc, char** argv)
{
    ...
    int64_t blockshape_x[] = {2000, 3000};
    ...
    int64_t cshape_out[] = {blockshape_x[0]};
    int64_t bshape_out[] = {500};
    ...
    printf("Vector Y has a shape of (%ld) with a chunk of (%ld) \n",
    ...
    if (INA_SUCCEED(ina_opt_isset("p")) && _iarray_path_exists(mat_x_prop.urlpath) && _iarray_path_exists(mat_y_prop.urlpath)) {
        ...
        INA_STOPWATCH_START(w);
        INA_MUST_SUCCEED(iarray_from_buffer(ctx, &xdtshape, mat_x, sizeof(double) * size_x,
                                            &mat_x_prop, &con_x));
        INA_MUST_SUCCEED(iarray_from_buffer(ctx, &ydtshape, mat_y, sizeof(double) * size_y,
                                            &mat_y_prop, &con_y));
        INA_STOPWATCH_STOP(w);
        INA_MUST_SUCCEED(ina_stopwatch_duration(w, &elapsed_sec));
    ...
    INA_STOPWATCH_START(w);
    iarray_linalg_matmul(ctx, con_x, con_y, &mat_out_prop, &con_out); /* FIXME: error handling */
    INA_STOPWATCH_STOP(w);
    INA_MUST_SUCCEED(ina_stopwatch_duration(w, &elapsed_sec));
