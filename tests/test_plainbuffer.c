...
static ina_rc_t test_plain(iarray_context_t *ctx,
...
INA_TEST_DATA(plainbuffer) {
    iarray_context_t *ctx;
};

INA_TEST_SETUP(plainbuffer)
{
    iarray_init();
...
INA_TEST_TEARDOWN(plainbuffer)
{
    iarray_context_free(&data->ctx);
    iarray_destroy();
}

INA_TEST_FIXTURE(plainbuffer, double_data)
{
    iarray_data_type_t dtype = IARRAY_DATA_TYPE_DOUBLE;
    size_t type_size = sizeof(double);

    int8_t ndim = 3;
    int64_t shape[] = {10, 20, 15};
    int64_t pshape[] = {0, 0, 0};
    double value = 3.1416;

    INA_TEST_ASSERT_SUCCEED(test_plain(data->ctx, dtype, type_size, ndim, shape, pshape, &value));
}
