...
typedef struct iarray_gemv_params_s {
...
static int _gemv_prefilter(blosc2_prefilter_params *pparams) {
    iarray_gemv_params_t *gparams = (iarray_gemv_params_t *) pparams->user_data;
    ...
    for (int b_nchunk = 0; b_nchunk < chunks_shape[1]; ++b_nchunk) {
        ...
        if (b_csize < 0) {
            IARRAY_TRACE1(iarray.tracing, "Error getting lazy b_blocks");
            return -1;
        }
...
INA_API(ina_rc_t) iarray_opt_gemv(iarray_context_t *ctx,
    ...
    if (b->catarr->ndim != 1) {
        IARRAY_TRACE1(iarray.error, "The b dimension must be 1");
        return INA_ERROR(IARRAY_ERR_INVALID_NDIM);
    }
    ...
    iarray_gemv_params_t gemv_params = {0};
    ...
    gemv_params.chunks_shape[0] = a->catarr->extshape[0] / a->catarr->chunkshape[0];
    gemv_params.chunks_shape[1] = a->catarr->extshape[1] / a->catarr->chunkshape[1];
