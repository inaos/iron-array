    ...
    while (iarray_iter_read_has_next(I2)) {
        ...
        switch (dtype) {
            case IARRAY_DATA_TYPE_DOUBLE:
                INA_TEST_ASSERT_EQUAL_FLOATING(val.elem_index_flatten * (stop - start) / (size - 1) + start,
                                               ((double *) val.pointer)[0]);
                break;
            case IARRAY_DATA_TYPE_FLOAT:
                INA_TEST_ASSERT_EQUAL_FLOATING((float) (val.elem_index_flatten * (stop - start) / (size - 1) + start),
                                               ((float *) val.pointer)[0]);
                break;
            default:
