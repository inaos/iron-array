...
static ina_rc_t test_gemm(iarray_context_t *ctx, iarray_container_t *c_x, iarray_container_t *c_y,
                          iarray_container_t *c_out, iarray_container_t *c_res, double tol) {
    INA_TEST_ASSERT_SUCCEED(iarray_matmul(ctx, c_x, c_y, c_out, IARRAY_OPERATION_GENERAL));
...
static ina_rc_t test_gemv(iarray_context_t *ctx, iarray_container_t *c_x, iarray_container_t *c_y,
                          iarray_container_t *c_out, iarray_container_t *c_res, double tol) {
    iarray_matmul(ctx, c_x, c_y, c_out, IARRAY_OPERATION_GENERAL);
    if (!iarray_almost_equal_data(c_out, c_res, tol)) {
        return INA_ERROR(INA_ERR_FAILED);
    }
    return INA_SUCCESS;
