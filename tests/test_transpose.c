...
static ina_rc_t test_transpose(iarray_context_t *ctx, iarray_data_type_t dtype, int32_t type_size, int8_t ndim,
    ...
    storage.backend = cshape ? IARRAY_STORAGE_BLOSC : IARRAY_STORAGE_PLAINBUFFER;
    ...
    INA_TEST_ASSERT_SUCCEED(iarray_container_new(ctx, &xdtshape, &storage, 0, &c_x));
...
INA_TEST_FIXTURE(iterator, 2_d_p) {
    ...
    INA_TEST_ASSERT_SUCCEED(test_transpose(data->ctx, dtype, type_size, ndim, shape, cshape, bshape));
}
