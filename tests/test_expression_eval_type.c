...
static int fill_x(float* x, int64_t nelem)
{
    /* Fill even values between 0. and 1. */
    float incx = 1 / (float) nelem;
...
static void fill_y(const float* x, double* y, int64_t nelem, double (func)(double))
{
    for (int i = 0; i < nelem; i++) {
        y[i] = func((double)x[i]);
    }
                                    ...
                                    char *urlpath)
{
    ...
    INA_TEST_ASSERT_SUCCEED(iarray_get_type_view(ctx, c_x, IARRAY_DATA_TYPE_DOUBLE, &c_view));

    INA_TEST_ASSERT_SUCCEED(iarray_expr_new(ctx, IARRAY_DATA_TYPE_DOUBLE, &e));
    if (func != const_) {
        INA_TEST_ASSERT_SUCCEED(iarray_expr_bind(e, "x", c_view));
    }
...
INA_TEST_DATA(expression_eval_type)
{
    iarray_config_t cfg;
    double (*func)(double);
    char *expr_str;
...
INA_TEST_SETUP(expression_eval_type)
{
    iarray_init();
...
INA_TEST_TEARDOWN(expression_eval_type)
{
    INA_UNUSED(data);
    iarray_destroy();
}
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc_constant)
{
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc_superchunk)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERBLOSC;
    data->func = expr_;
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc2_superchunk)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERBLOSC;
    data->func = expr_;
    data->expr_str = "(x - 2.3) * (x - 1.35) * (x + 4.2)";
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc_superchunk0)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERBLOSC;
    data->func = expr0;
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc_superchunk1)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERBLOSC;
    data->func = expr1;
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc_superchunk_memcpy)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERBLOSC;
    data->func = expr1;
    // eval of constants is not supported with the interpreter engine
...
INA_TEST_FIXTURE(expression_eval_type, iterchunk_superchunk2)
{
...
INA_TEST_FIXTURE(expression_eval_type, iterchunk_superchunk3)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERCHUNK;
    data->func = expr3;
...
INA_TEST_FIXTURE(expression_eval_type, default_superchunk2)
{
...
INA_TEST_FIXTURE(expression_eval_type, llvm_dup_trans)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_AUTO;
    data->func = expr4;
...
INA_TEST_FIXTURE(expression_eval_type, iterchunk_atan2)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERCHUNK;
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc_pow2)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERBLOSC;
    data->func = expr_pow2;
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc_power_pi)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERBLOSC;
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc_min)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERBLOSC;
    data->func = expr_min;
...
INA_TEST_FIXTURE(expression_eval_type, iterblosc_max)
{
    data->cfg.eval_method = IARRAY_EVAL_METHOD_ITERBLOSC;
    data->func = expr_max;
